...
const Footer = () =>  (
    ...
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
                ...
                <div key={footerLinks.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
                   ...
                   <ul className='list-none mt-4'>
                      {footerLinks.links.map((link, index) => (
                        <li key={link.name} className={`font-poppins font-normal leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index !== footerLinks.links.length - 1 ? 'mb-4' : 'mb-0'}`}>
                          {link.name}
                        </li>
                      ))}
                   </ul>
      ...
      <div className='w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3r45]'>
        <p className='font-poppins font-normal text-[18px] text-center leading-[27px] text-white'>2024 HooBank. All Rights Reserved. Made by Abdou Salam 💻🤍</p>
